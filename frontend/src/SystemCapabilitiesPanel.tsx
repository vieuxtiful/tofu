...
export default function SystemCapabilitiesPanel({ onClose }: { onClose: () => void }) {
  ...
  const groups: Array<{ title: string; items: Array<[string, CapabilityStatus]> }> = data ? [
    ...
    },
    {
      title: "Capture",
      items: [
        ...data.ocr.providers.map((p) => [p.id, p] as [string, CapabilityStatus]),
        ...data.scene.providers.map((p) => [p.id, p] as [string, CapabilityStatus]),
      ],
    },
    {
      title: "Render",
      items: [
        ...data.shaping.providers.map((p) => [p.id, p] as [string, CapabilityStatus]),
        ...data.inpainting.providers.map((p) => [p.id, p] as [string, CapabilityStatus]),
      ],
    },
    {
      title: "Translate",
      items: data.semantics.providers.map((p) => [p.id, p] as [string, CapabilityStatus]),
    },
    {
      title: "Translation",
      items: data.translation.providers.map((p) => [p.id, p] as [string, CapabilityStatus]),
    },
  ] : [];
