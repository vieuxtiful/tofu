...
export default function SmartFillReview({ reviews, fallbackIds, previews, previewPending, appliedIds, onRetryPreview, onApply, onApplyAll, onHoverRegion }: Props) {
  ...
  return <div ref={panelRef} className="mt-2 rounded border border-amber-500/40 bg-amber-50/70 px-2 py-2 text-xs text-amber-900 dark:bg-amber-950/20 dark:text-amber-100" onMouseLeave={() => onHoverRegion?.(null)}>
    <div className="flex items-center gap-2">
      <button type="button" onClick={() => { onHoverRegion?.(null); setOpen((value) => !value); }} className="flex w-full items-center gap-1 rounded px-1 py-0.5 text-left font-semibold transition hover:bg-amber-100/70 dark:hover:bg-amber-950/40" aria-expanded={open}>
    ...
    <div className={`smart-fill-content${open ? " expanded" : ""}`}>
      ...
      <div className="mt-1.5 flex justify-end"><button type="button" onClick={onApplyAll} disabled={readyCandidates.length === 0} className="rounded bg-amber-700 px-1.5 py-0.5 text-[10px] text-white disabled:cursor-not-allowed disabled:opacity-50">Apply All</button></div>
