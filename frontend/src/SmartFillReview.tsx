...
export default function SmartFillReview({ reviews, fallbackIds, dismissed, previews, previewPending, appliedIds, onDismiss, onRetryPreview, onApply }: Props) {
  ...
  return <div className="mt-2 rounded border border-amber-500/40 bg-amber-50/70 px-2 py-2 text-xs text-amber-900 dark:bg-amber-950/20 dark:text-amber-100">
    ...
    {fallbackIds.length > 0 && <div className="mb-1 rounded border border-sky-500/30 bg-sky-50/50 px-2 py-1 text-[10px] text-sky-900 dark:bg-sky-950/20 dark:text-sky-100"><span className="font-medium">Texture treatment is ready for review.</span><span className="ml-1">{fallbackIds.length} region{fallbackIds.length === 1 ? " uses" : "s use"} the editable reconstruction base.</span></div>}
    {candidates.length > 0 && <><span className="font-medium">Smart fill:</span><span className="ml-1">Compare a proposed fill with the localized canvas, then apply it only if it improves the surface.</span>
