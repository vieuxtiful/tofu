...
class TestDetParamForwarding:
    """regression: det_db_thresh/drop_score were declared on __init__ but
    never actually reached the worker's request payload, so there was no
    way to tune DBNet's box tightness at all (Cluster 2 of the japan-
    street/china-street accuracy hand-off)."""
    ...
    def test_unset_optional_params_omitted(self):
        ...
        # guessed value the worker would misinterpret as an explicit ask
        backend = PaddleOCRBackend(languages=["ja"])
        assert "det_box_thresh" not in backend._det_params()
        assert "unclip_ratio" not in backend._det_params()

    def test_explicit_unclip_ratio_included(self):
        backend = PaddleOCRBackend(languages=["ja"], unclip_ratio=1.9, det_box_thresh=0.5)
        params = backend._det_params()
        assert params["unclip_ratio"] == 1.9
        assert params["det_box_thresh"] == 0.5

    def test_detect_request_includes_det_params(self, monkeypatch):
        backend = PaddleOCRBackend(languages=["ja"], unclip_ratio=1.9)
        captured = {}
        def fake_run_worker(request):
            captured.update(request)
            return {"ok": True, "detections": []}
        monkeypatch.setattr(backend, "_run_worker", fake_run_worker)
        backend.detect("some/path.png")
        assert captured["unclip_ratio"] == 1.9
        assert captured["det_db_thresh"] == 0.3

    def test_detect_in_regions_request_includes_det_params(self, monkeypatch):
        from tofu.core.types import BBox
        backend = PaddleOCRBackend(languages=["ja"], det_box_thresh=0.5)
        captured = {}
        def fake_run_worker(request):
            captured.update(request)
            return {"ok": True, "per_region": [[]]}
        monkeypatch.setattr(backend, "_run_worker", fake_run_worker)
        backend.detect_in_regions("some/path.png", [BBox(x=0, y=0, width=10, height=10)])
        assert captured["det_box_thresh"] == 0.5
