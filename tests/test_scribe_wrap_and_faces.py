...
ARIAL_WITH_HEAVY = ARIAL_FAMILY + [
    ("arialheavy.ttf", "Arial", "Heavy", 900),
]
...
class TestResolveFace:
    ...
    def test_heavy_or_black_request_uses_a_900_face(self):
        reg = make_registry(ARIAL_WITH_HEAVY)
        heavy, synth_heavy = scribe.resolve_face(reg, "arial.ttf", "Heavy", False)
        black, synth_black = scribe.resolve_face(reg, "arial.ttf", "Black", False)
        assert (heavy, synth_heavy) == ("arialheavy.ttf", False)
        assert (black, synth_black) == ("arialheavy.ttf", False)
