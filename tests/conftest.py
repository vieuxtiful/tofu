...
from tofu.utils.manifest_store import set_save_guard  # noqa: E402
...
def manifest_save_integrity_guard():
    def guard(manifest, asset_id):
        assert_manifest_integrity(manifest, context=f"save {asset_id}")

    previous = set_save_guard(guard)
    yield
