...
def manifest_from_gt(name: str, region_idx: int = 0, texture: str | None = None):
    ...
    surfaces = []
    if texture:
        surfaces.append(SceneRegion(
            bbox=bbox, semantic_label="panel", confidence=1.0,
            texture=texture,
        ))
    manifest = TextManifest(asset_id="a", total_regions=1, instances=[inst], scene_regions=surfaces)
...
class TestSceneCleanseAgreement:
    def test_agreeing_flat_panel_uses_flat_fill(self):
        img, manifest, _ = manifest_from_gt("flat-sign", 0, texture="flat")
        cleanse.erase(img, manifest)
        profile = manifest.instances[0].background_profile
        assert profile.surface_texture == "flat"
        assert profile.cleanse_strategy == "flat"

    def test_disagreeing_surface_never_keeps_the_analytic_flat_fill(self):
        img, manifest, _ = manifest_from_gt("flat-sign", 0, texture="flat")
        manifest.scene_regions[0].texture = "textured"
        cleanse.erase(img, manifest)
        # The scene/instance agreement gate must prevent a cheap flat fill
        # from being trusted.  A locally provisioned neural provider now gets
        # the first review-only candidate; installations without one use the
        # deterministic Telea fallback.  Both paths are valid so long as the
        # original analytic strategy is not silently retained.
        strategy = manifest.instances[0].background_profile.cleanse_strategy
        assert strategy in {"neural", "telea"}
        assert strategy != "flat"

    def test_polygon_clip_never_marks_outside_surface(self):
        import cv2
        from tofu.layers.cleanse import _clip_to_polygon
        mask = np.ones((20, 20), dtype=bool)
        clipped = _clip_to_polygon(np, cv2, mask, [(0, 0), (19, 0), (0, 19)], 20, 20)
        assert clipped[2, 2]
        assert not clipped[16, 16]
